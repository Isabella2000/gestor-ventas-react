...
const JsonToHtml = () => {
  ...
  return (
    <div className="w-100 h-100 flex justify-around items-center mt-20 flex-col">
      <div
        className=" bg-amber-200 rounded-md flex justify-around items-center mb-10 pl-10"
        style={{
          width: "1400px",
        }}
      >
        <div className="">
          <h1 className="text-5xl font-bold pb-6">
            Gestor <span className="text-orange-500">Empresarial</span>
          </h1>

          <div className="flex justify-around items-start flex-col capitalize ">
            {empresaGestoraVentasJSON.map((e, index) => (
              <CardEmpresaGestora key={index} e={e} />
            ))}
          </div>
        </div>

        <img
          src="https://yellowdesk.imgix.net/images/Yellow-Desk-Cowork_Finals_SimonShiff_0-31.jpg?crop=focalpoint&domain=yellowdesk.imgix.net&fit=crop&fm=pjpg&fp-x=0.5&fp-y=0.5&h=675&ixlib=php-3.3.1&q=82&w=1200&s=35ad89d58472093427c58e6b5643bd82"
          alt=""
          className="w-2/4 py-10"
      ...
      <h1 className="text-5xl font-bold py-6">
        Empresas <span className="text-orange-500">Clientes</span>
      </h1>

      <div className="flex justify-around items-start gap-20">
        {data.empresasClientes.map((d) => (
          <CardEmpresaClientes key={d.nombreEmpresa} d={d} />
        ))}
      </div>
